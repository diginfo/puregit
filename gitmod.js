...
$.__src = $.__src || '/usr/share/dev/users/pac/api/src/pure3';
$.__run = $.__run || '/usr/share/dev/users/pac/api/run/pure3';
...
function push(dir,cb=cl){
  git_exec(dir,{args:["log","origin/master..HEAD"]},function(raw){
    if(!raw || !raw.length) {
      if(mex.debug > 0) cl('Nothing to push.');
      return cb();
    }
    if(mex.debug > 0) cl('Pushing changes...');
...
function fileAdd(dir,file,cb=cl){
  git_exec(dir,{args:["add",file.name]},function(res){
    if(mex.debug > 1) cl('fileAdd()',file);
    if(cb) cb(res);
  })         
}
...
function filePublish(dir,file,cb=cl){
  fileAdd(dir,file,function(ok){
    if(mex.debug > 1) cl('filePublish()',file);
...
function ver2array(data){
  ...
  maj=parseInt(maj||mex.version);min=parseInt(min||'0'); 
  return[maj,min];    
}
...
function fileCommit(dir,file,cb=cl){
  ...
  if(mex.debug > 1) cl('fileCommit():',notes);
  ...
  async.eachSeries(files,function(file,next){
    ...
    */
    ...
    if((/D/).test(file.stat)){
      ...
      fileRm($.__run,file,function(rm){
        fileCommit($.__src,file,function(com){
          files.splice(idx,1);
          next();  
        }) 
      });
...
function infoGet(fns,cb=cl){
  ...
  async.eachSeries(fns,function(fn,next){
    ...
    git_exec($.__src,{args:cmd},function(list){
